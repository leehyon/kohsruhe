...
这两年大模型发展很快，完全跟不上。越看越觉得，单纯从 Transformer[^ud2]、提示词工程（Prompt Engineering）、缰绳系统（Harness Engineering）这些概念入手，很容易陷入细节、落个「只缘身在此山中」。倒不如后退一步，问一个更基础的问题：
...
[^ud2]: Transformer 是一种基于多头注意力机制的人工神经网络架构

---

## 从序列建模说起

最早接触「序列」这个概念还是刚工作那会儿在做胎压监测算法的时候。当时公司的核心产品，就是基于每个轮胎过去几分钟的胎压变化，去预测是否存在漏气风险。它本质上是一个典型的时间序列问题。那也是我第一次直观地感受到：序列里蕴含的信息，远比我们想象的要多。
